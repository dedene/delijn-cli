...
const (
	apiKeyKey          = "api_key"
	keyringPasswordEnv = "DELIJN_KEYRING_PASSWORD" //nolint:gosec // env var name
	keyringBackendEnv  = "DELIJN_KEYRING_BACKEND"  //nolint:gosec // env var name
	apiKeyEnv          = "DELIJN_API_KEY"          //nolint:gosec // env var name
)
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set DELIJN_KEYRING_BACKEND=file and DELIJN_KEYRING_PASSWORD=<password> to use encrypted file storage instead",
